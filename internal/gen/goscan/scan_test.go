...
func TestParseTags(t *testing.T) {
	tests := []struct {
		...
		{name: "ondelete", tag: `orm:"ondelete:cascade"`, want: model.FieldTags{OnDelete: "CASCADE", Raw: "ondelete:cascade"}},
		{name: "onupdate", tag: `orm:"onupdate:restrict"`, want: model.FieldTags{OnUpdate: "RESTRICT", Raw: "onupdate:restrict"}},
		{name: "two-word action has no space", tag: `orm:"ondelete:setnull"`, want: model.FieldTags{OnDelete: "SET NULL", Raw: "ondelete:setnull"}},
		{name: "action case does not matter", tag: `orm:"ondelete:CASCADE"`, want: model.FieldTags{OnDelete: "CASCADE", Raw: "ondelete:CASCADE"}},
		{name: "fk with an action", tag: `orm:"fk:c,ondelete:cascade"`, want: model.FieldTags{FK: "c", OnDelete: "CASCADE", Raw: "fk:c,ondelete:cascade"}},
		{name: "bad action", tag: `orm:"ondelete:explode"`, wantErr: "not a referential action"},
		{name: "action written with a space", tag: `orm:"ondelete:set null"`, wantErr: "not a referential action"},
