...
const copy = {
  en: {
    ...
    lede:
      "Neither representation is generated from the other. You write the structs; migrations own the schema. The orm command introspects both, reports every place they disagree, and generates typed metadata only from a mapping it proved.",
    start: "Get started",
  ...
  ru: {
    ...
    lede:
      "Ни одно представление не порождается из другого. Структуры пишете вы, схемой владеют миграции. Команда orm читает и то и другое, показывает каждое расхождение и генерирует типизированные метаданные только из отображения, которое смогла доказать.",
    start: "Начать",
    ...
    <main id="main">
          ...
          <div>
            ...
            <h1 className="text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl lg:text-6xl">
              {c.thesis.map((line, i) => (
                <span
                  key={i}
                  className={i === 2 ? "block bg-gradient-to-r from-[var(--color-go-blue)] to-[var(--color-go-light)] bg-clip-text text-transparent" : "block"}
                >
                  {line}
                </span>
              ))}
          ...
          <div className="relative mx-auto w-full max-w-sm lg:max-w-none">
            <div className="glass rounded-3xl p-8">
              <GopherWithDatabase
                ...
                label={locale === "ru" ? "Гофер с базой данных" : "A gopher holding a database"}
              />
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-[100rem] px-4 pb-16 sm:px-6">
        <div className="glass mx-auto max-w-4xl overflow-hidden rounded-2xl">
          <div className="flex items-center gap-2 border-b border-[var(--rule)] px-4 py-2.5">
            ...
            <span className="ml-2 font-mono text-xs text-[var(--fg-faint)]">{c.sampleTitle}</span>
          </div>
          <div
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {c.features.map((f) => (
            ...
            >
              ...
              <p className="text-sm leading-relaxed text-[var(--fg-muted)]">{f.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[100rem] px-4 pb-24 sm:px-6">
        <div className="glass grid items-center gap-8 rounded-3xl p-8 sm:p-12 lg:grid-cols-[auto_1fr]">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight">{c.proofTitle}</h2>
            <p className="mt-3 max-w-2xl leading-relaxed text-[var(--fg-muted)]">{c.proofBody}</p>
            <dl className="mt-6 flex flex-wrap gap-8">
              {c.stats.map((s) => (
                <div key={s.l}>
                  <dt className="text-3xl font-bold text-[var(--accent)]">{s.n}</dt>
