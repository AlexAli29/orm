...
Арность выписана — от `Project1` до `Project8`, — потому что Go не умеет сказать «список выражений, у которых все типы разные и все запомнены». Взамен горячий путь строки не делает рефлексии и не знает про `[]any`.
