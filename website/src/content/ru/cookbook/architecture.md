...
Хендлеры принимают `*domain.DB` напрямую. Слоя репозиториев нет, потому что на таком размере интерфейс репозитория с одной реализацией — это файл, который сопровождают просто так.

Пора менять, когда в хендлерах заводятся бизнес-правила или когда их хочется тестировать без базы.
