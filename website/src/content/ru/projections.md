...
## Число аргументов выписано явно

От `Project1` до `Project8` — арность, то есть число принимаемых аргументов, задана каждым конструктором отдельно. Go не умеет сказать «список выражений, у которых все типы разные и все запомнены»: у вариадика один тип, а пакета параметров типа не существует. Каждая библиотека, которая делает вид, что умеет, делает это через `[]any` и приведения в рантайме — то есть переносит ошибку с компилятора на пользователя.

Что это даёт: горячий путь строки не делает рефлексии, не держит карту и ничего не приводит. Сканирование — это N типизированных локальных переменных, один `Scan` и один вызов.
